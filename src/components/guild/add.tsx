...
export default function Add(props: Iprops) {
  ...
  const roleOptions = useMemo(() => {
    return [
      { label: t('Guild.Moderator'), value: UserRole.Admin },
      { label: t('Guild.Members'), value: UserRole.Member },
    ];
  }, [t]);

  return (
    <AddMemberModal title={t('Guild.AddMember')} handleClose={closeAdd}>
      <CardBody>
        <InnerBox>
          <ul>
            <InputLableRow>
              <InputBox>{t('Guild.AddMemberAddress')}</InputBox>
              <div style={{ width: '140px' }}>{t('Guild.MemberRole')}</div>
              <OptionBox></OptionBox>
            </InputLableRow>
            {lst.map((item, index) => (
              <li key={index}>
